...
class PictureAdditionTest : public ::testing::Test,
  ...
  public:
    ...
    void SetUp() override {
        ...
        recon1_ = reinterpret_cast<uint8_t *>(eb_aom_memalign(32, test_size_));
        recon2_ = reinterpret_cast<uint8_t *>(eb_aom_memalign(32, test_size_));
        ...
        recon2_16bit_ = reinterpret_cast<uint16_t *>(
            eb_aom_memalign(32, sizeof(uint16_t) * test_size_));
        memset(recon1_, 0, test_size_ * sizeof(recon1_[0]));
        memset(recon2_, 0, test_size_ * sizeof(recon2_[0]));
  ...
  protected:
    void prepare_data() {
        const uint8_t mask_uint8 = (1 << 8) - 1;
        const int16_t mask_int16 = (1 << 15) - 1;
        SVTRandom rnd_uint8(0, mask_uint8);
        SVTRandom rnd_int16(0, mask_int16);
        switch (test_pattern_) {
        case PRED_MIN: {
            for (uint32_t i = 0; i < test_size_; i++) {
                pred_[i] = 0;
                residual_[i] = residual_int32_[i] = mask_int16;
            }
            break;
        }
        case PRED_MAX: {
    ...
    void prepare_16bit_data() {
        const uint16_t mask_uint16 = (1 << 16) - 1;
        const int16_t mask_int16 = (1 << 15) - 1;
        SVTRandom rnd_uint16(0, mask_uint16);
        SVTRandom rnd_int16(0, mask_int16);
        switch (test_pattern_) {
        case PRED_MIN: {
            for (uint32_t i = 0; i < test_size_; i++) {
    ...
    void check_output(uint32_t width, uint32_t height) {
        int fail_count = 0;
        for (uint32_t j = 0; j < height; j++) {
            for (uint32_t k = 0; k < width; k++) {
                if (recon1_[k + j * recon_stride_] !=
                    recon2_[k + j * recon_stride_])
                    fail_count++;
            }
        }
    ...
    void run_test() {
        ...
        check_output(area_width_, area_height_);
