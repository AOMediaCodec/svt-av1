 ...
 */
static inline EbBool get_non_scaling_spatial_amvp(
    PredictionUnit_t     *neighbour_pu_ptr,           // input parameter, please refer to the detailed explanation above.
    EbReflist            target_ref_pic_list,        // input parameter, the reference picture list where the AMVP is searching for.
    uint64_t              target_ref_pic_poc,         // input parameter, the POC of the reference picture where the AMVP is searcing for.
    int16_t              *mvp_cand_x,                 // output parameter, the horizontal componenet of the output AMVP candidate.
    int16_t              *mvp_cand_y,                 // output parameter, the vertical componenet of the output AMVP candidate.
    PictureControlSet_t  *picture_control_set_ptr)
{
    EbBool                  availability = EB_FALSE;
    ...
    CHECK_REPORT_ERROR(
        (neighbour_pu_ptr != EB_NULL),
        encode_context_ptr->app_callback_ptr,
        EB_ENC_AMVP_ERROR2);

    if (neighbour_pu_ptr != EB_NULL) {
        switch (neighbour_pu_ptr->inter_pred_direction_index) {
        case UNI_PRED_LIST_0:

            reference_object = (EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_0]->objectPtr;
            ref_pic_poc = reference_object->refPOC;
            availability = (EbBool)(target_ref_pic_poc == ref_pic_poc);
            *mvp_cand_x = neighbour_pu_ptr->mv[REF_LIST_0].x;
            *mvp_cand_y = neighbour_pu_ptr->mv[REF_LIST_0].y;

            break;

        case UNI_PRED_LIST_1:

            reference_object = (EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[REF_LIST_1]->objectPtr;
            ref_pic_poc = reference_object->refPOC;
            availability = (EbBool)(target_ref_pic_poc == ref_pic_poc);
            *mvp_cand_x = neighbour_pu_ptr->mv[REF_LIST_1].x;
            *mvp_cand_y = neighbour_pu_ptr->mv[REF_LIST_1].y;

            break;

        case BI_PRED:

            // Check the AMVP in target_ref_pic_list
            ref_pic_list = target_ref_pic_list;
            availability = (EbBool)((neighbour_pu_ptr->inter_pred_direction_index == BI_PRED) || (neighbour_pu_ptr->inter_pred_direction_index == (uint8_t)ref_pic_list));
            reference_object = (EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[ref_pic_list]->objectPtr;

            if (neighbour_pu_ptr) {

                ref_pic_poc = reference_object->refPOC;
                availability = (EbBool)(availability &  (target_ref_pic_poc == ref_pic_poc));
                if (availability) {
                    *mvp_cand_x = neighbour_pu_ptr->mv[ref_pic_list].x;
                    *mvp_cand_y = neighbour_pu_ptr->mv[ref_pic_list].y;
                }
                else {
                    // Check the AMVP in 1 - target_ref_pic_list
                    ref_pic_list = (EbReflist)(1 - target_ref_pic_list);
                    availability = (EbBool)((neighbour_pu_ptr->inter_pred_direction_index == BI_PRED) || (neighbour_pu_ptr->inter_pred_direction_index == (uint8_t)ref_pic_list));
                    reference_object = (EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[ref_pic_list]->objectPtr;
...
/** get_non_scaling_spatial_amvp_v2()
        is used to generate the spatial AMVP candidate by using the non-scaling method.

    It returns the availability of the spatial AMVP candidate.
 */
static inline EbBool get_non_scaling_spatial_amvp_v2(
    MvUnit_t               *mv_unit,                     // input parameter, please refer to the detailed explanation above.
    EbReflist               target_ref_pic_list,         // input parameter, the reference picture list where the AMVP is searching for.
    uint64_t                target_ref_pic_poc,          // input parameter, the POC of the reference picture where the AMVP is searcing for.
    int16_t                *mvp_cand_x,                  // output parameter, the horizontal componenet of the output AMVP candidate.
    int16_t                *mvp_cand_y,                  // output parameter, the vertical componenet of the output AMVP candidate.
    PictureControlSet_t    *picture_control_set_ptr)
{
    ...
    case UNI_PRED_LIST_0:
        ...
        availability = (EbBool)(target_ref_pic_poc == ref_pic_poc);
        if (availability) {
            *mvp_cand_x = mv_unit->mv[REF_LIST_0].x;
            *mvp_cand_y = mv_unit->mv[REF_LIST_0].y;
        }
    ...
    case UNI_PRED_LIST_1:
        ...
        availability = (EbBool)(target_ref_pic_poc == ref_pic_poc);
        if (availability) {
            *mvp_cand_x = mv_unit->mv[REF_LIST_1].x;
            *mvp_cand_y = mv_unit->mv[REF_LIST_1].y;
        }
    ...
    case BI_PRED:
        ...
        // Check the AMVP in target_ref_pic_list
        reference_object = (EbReferenceObject_t*)picture_control_set_ptr->ref_pic_ptr_array[target_ref_pic_list]->objectPtr;
        ref_pic_poc = reference_object->refPOC;
        availability = (EbBool)(target_ref_pic_poc == ref_pic_poc);
        if (availability) {
            *mvp_cand_x = mv_unit->mv[target_ref_pic_list].x;
            *mvp_cand_y = mv_unit->mv[target_ref_pic_list].y;
        }
        else {
            // Check the AMVP in 1 - target_ref_pic_list
            ref_pic_list = (EbReflist)(1 - target_ref_pic_list);
            ...
            ref_pic_poc = reference_object->refPOC;
            availability = (EbBool)(target_ref_pic_poc == ref_pic_poc);
            *mvp_cand_x = mv_unit->mv[ref_pic_list].x;
            *mvp_cand_y = mv_unit->mv[ref_pic_list].y;
    ...
    PictureControlSet_t    *picture_control_set_ptr)
{
    ...
    if (mv_unit->predDirection == BI_PRED)
        ...
        ref_pic_list = (EbReflist)mv_unit->predDirection;
...
/** get_scaling_spatial_amvp()
        is used to generate the spatial AMVP candidate by using the scaling method.
 */
static inline EbBool get_scaling_spatial_amvp(
    PredictionUnit_t     *neighbour_pu_ptr,                // input parameter, please refer to the detailed explanation above.
    EbReflist             target_ref_pic_list,             // input parameter, the reference picture list where the AMVP is searching for.
    uint64_t              target_ref_pic_poc,              // input parameter, the POC of the reference picture where the AMVP is searcing for.
    int16_t              *mvp_cand_x,                      // output parameter, the horizontal componenet of the output AMVP candidate.
    ...
    PictureControlSet_t  *picture_control_set_ptr)
{
    ...
    EbReferenceObject_t    *reference_object;
    uint64_t                ref_pic_poc;
    EbReflist               ref_pic_list = REF_LIST_0;
    ...
    case UNI_PRED_LIST_0:
        ...
        ref_pic_list = REF_LIST_0;
        break;

    case UNI_PRED_LIST_1:

        ref_pic_list = REF_LIST_1;
        break;

    case BI_PRED:

        availability = (EbBool)((neighbour_pu_ptr->inter_pred_direction_index == BI_PRED) || (neighbour_pu_ptr->inter_pred_direction_index == (uint8_t)target_ref_pic_list));
        ref_pic_list = availability ? target_ref_pic_list : (EbReflist)(1 - target_ref_pic_list);
        break;
