...
*/
double eb_av1_compute_cross_correlation_c(unsigned char *im1, int stride1, int x1,
    ...
    for (y = -SEARCH_SZ_BY2; y <= SEARCH_SZ_BY2; ++y) {
      for (x = -SEARCH_SZ_BY2; x <= SEARCH_SZ_BY2; ++x) {
        ...
        if (!is_eligible_distance(correspondences[i].x, correspondences[i].y,
                                  ...
                                  correspondences[i].ry + y, width, height))
          continue;
        match_ncc = eb_av1_compute_cross_correlation(
...
int eb_av1_determine_correspondence(unsigned char *frm, int *frm_corners,
